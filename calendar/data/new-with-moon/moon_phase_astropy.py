...
df = pd.read_csv('orthodox_feasts.csv', sep='\t', quoting=1, keep_default_na=False)  # keep_default_na=False prevents empty strings from becoming NaN
...
for date in df['date']:
    ...
    # Only record the phase if it's different from the previous day and not 0
    if current_phase != 0 and current_phase != prev_phase:
